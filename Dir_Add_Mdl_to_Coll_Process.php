...
<?php
    if (isset($_POST['var_coll_submit'])) {
        ...
        $query = "INSERT INTO Matchbox_Collection (Username, User_Coll_ID, UMID, VerID, VarID, RelID, User_SpecID, Copy, VehCond, PkgCond, ItemVal, StorLoc, StorLoc2, PurchDt, Seller, PurchPrice, SellFlag, MinSellPrice, CollComm, Coll_InactiveFlg) VALUES ('$Coll_Username','$User_CollID','$UMID', '$VerID', '$VarID', '$RelID', '$User_SpecID', '$Copy', '$VehCond', '$PkgCond', '$ItemVal', '$StorLoc', '$StorLoc2', '$PurchDt', '$Seller', '$PurchPrice', '$SellFlg', '$MinSellPric', '$CollComm', '$Coll_InactiveFlg')";
	...
	<div class="large-12 columns">
		...
		<form name="Dir_Add_Mdl_to_Coll_Process" action="Dir_Add_Mdl_to_Coll_Process.php" method="post">
            ...
            ?>
				...
				<?php
			    	$query=("SELECT * FROM Matchbox_User_Coll_Value_lists WHERE Username='$User' AND User_Coll_ID LIKE '%$User_CollID%' AND Coll_List_Type LIKE '%Location%' AND (!Coll_List_Val_InactivFlg) ORDER BY Coll_List_Val_DisplOrd ASC");								
	                $result=0;
	                $rows_count=0;									
	                $result = mysql_query($query);
	
	                if ((mysql_num_rows($result) == 0)) {
	                    echo "<input type=\"text\" name=\"Coll_Loc1\" value=\"\" id=\"Coll_Loc1\">";
	                } else { ?>  
				...
				<?php
                    ...
                    if ((mysql_num_rows($result) == 0)) {
                        echo "<input type=\"text\" name=\"Coll_Loc2\" value=\"\" id=\"Coll_Loc2\">";
                    } else { ?>  
				...
				<?php
			    	$query=("SELECT * FROM Matchbox_User_Coll_Value_lists WHERE Username='$Username' AND User_Coll_ID LIKE '%$User_CollID%' AND Coll_List_Type LIKE '%Seller%' AND (!Coll_List_Val_InactivFlg) ORDER BY Coll_List_Val_DisplOrd ASC");								
                    $result=0;
                    $rows_count=0;									
                    $result = mysql_query($query);

                    if ((mysql_num_rows($result) == 0)) {
                        echo "<input type=\"text\" name=\"Coll_Seller\" value=\"\" id=\"Coll_Seller\">";
                    } else { ?>  
				...
				<input type="checkbox" id="cbxCollSellFlg" name="CollSellFlg"><label for="cbxCollSellFlg">Flag to Sell?</label>				
				
				
				<label for="Coll_MinSell_Price">Minimum Price to Sell</label>
				<input type="text" name="Coll_MinSell_Price" value="" id="Coll_MinSell_Price">
				...
				?>
				<a href="<?php echo $url; ?>" class="button dark cancel">Cancel</a>
				
                <!--label for="REPLACE">Username</label-->
