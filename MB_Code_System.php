...
<div class="row">
	<div class="large-12 columns">
		...
		<p>Code 4 are one-off models created by Matchbox for charities or auctions or to recognize individuals. The gold buses given to each year's Matchbox Ambassador (a selected liaison from the collecting community to key Matchbox team members) and the gold-plated 2003 Seagrave Meanstick Firetruck auctioned for The Ronald McDonald House charities are examples of these.</p>
		<p>The line shifted between Code 2 & 3 models several years ago with changes in 'permission' from Mattel. Previous to circa 2006, models finished by a Code 2 authorized <i>company</i> (such as CCI and ASAP) were considered code 2. Since then only specific <i>models</i> that have the permission of Mattel are considered code 2. Models continuing to be made by CCI, based on either old blanks or standard release models, are now considered Code 3 since the change. Of late, Mattel has distanced itself (for legal reasons- not wanting to have any accountability for whatever message people put on their toys) from the process- they no longer produce blanks and the only Code 2 authorized models are a handful made for charities and some specific toy shows.</p>
		<p>Despite this, by popular demand, CCI produces many customer-driven prints, using the remaining blanks or regular models, which the customers often still think of as Code 2 (as they were prior to 2006 or so). How to categorize these? Since we don't have a sharp definition of when Code 2 shifted to Code 3 for CCI and others, and considering the continuing popularity and collectibility of many (now) code 3 models, I've fudged in this database and list them all as Code 2/3. In this I include only the old 'true' Code 2, as well as current Code 3 made by the previously authorized companies and clubs/groups. If you customized it in your basement- not included!</p>
		
		<p>So to summarize, on this site:</p>
		<ul>
			<li>Code 1 are any model made entirely by Matchbox or at a Matchbox factory</li>
			<li>Code 2/3 are Code 1 models that are further decorated/printed by a secondary manufacturer either a) with specific permission of Mattel to make that Code 2 model, or b) by a company/group who has been authorized by Matchbox previously to create Code 2's, which now are considered Code 3.</li>
		...
		<p>In general the value of a model is based on a combination of quantity available and code level, with level 4 being by far the most expensive if they are ever made available by the original receiver).</p>
